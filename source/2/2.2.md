---
description: After setting up your GoCD environment, here are some instructions on how to get started making changes to the GoCD codebase.
keywords: gocd contributions, open source contributions, gocd development environment, submitting gocd fixes
---
...
If your development environment is not set up, please refer to [Setting up your development environment](2.1.md) for the steps to set up the GoCD development environment.
...
- Fix the failing spec to match the expectation. In our use case, because localization in GoCD uses the ```localize.xml``` file to interpolate string values, we will make appropriate changes in that file.
...
Once you are ready with the fix, you can submit your fix to the GoCD Team by means of [Github pull requests](https://help.github.com/articles/using-pull-requests). The approval process can be seen [here](https://www.gocd.org/contribute/contribution-guide.html#review-process).
